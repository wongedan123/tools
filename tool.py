...
import random
import nmap,json,html2text,subprocess,os,sys,mechanize,requests,urllib
...
def admin(host):
		sys.stdout.write(host + " Scanning...\r")
		sys.stdout.flush()
...
def sub(host):
		...
		try:
				...
				print (p + "[" + m + "!" + p + "]" + g + "ERROR: " + str(er.reason))
				sleep(1)
		except KeyboardInterrupt:
				pass
		pilih()
...
def dos(host):
    ...
    conn = raw_input(p + "[" + b + "+" + p + "]" + g + "Enter the number of packets to be sent(depends on the site but should be more than 2000 or 3000 for average sites): ")
    silit = raw_input(p + "[" + b + "+" + p + "]" + g + "Enter your message: ")
    conn = int(conn)
    subprocess.call("clear", shell = True)
    print ("please wait process FLODDING => %s") % (host)
    sleep(3)
...
def imdb(movie):
    ...
    m = urlopen('http://www.imdb.com/find?ref_=nv_sr_fn&q=' + movie.replace(' ','+') + '&s=all')
    m = m.read()
    for i in m.split('\n'):
        if "result_text" in i:
            if "/title/" in i:
...
def scanner(host):
		...
		if __name__ == '__main__':
			...
			print g + "please wait proccess scanning %s..." % (host)
			sleep(5)
...
def banner(host):
    ...
    port = raw_input(p + "[" + b + "*" + p + "]" + g + " Enter the port of the service: ")
    try:
        s.connect((host,int(port)))
        print p + "[" + b + "*" + p + "]" + g + " connection successfull\nWaiting for the banner...\n"
        sleep(2)
        if int(port) == 80:
...
def defense():
    ...
    print (p + "[" + g + "+" + p + "]" + g + " Berhasil Mendapatkan ID Teman..." + r)
...
def main():
    ...
    print m + "[" + p + "02" + m + "]" + g + ".DDOS" + r
    print m + "[" + p + "03" + m + "]" + g + ".HTTP headers" + r
